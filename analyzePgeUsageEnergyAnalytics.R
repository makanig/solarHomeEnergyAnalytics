...
electricUsageHourly = "pge_electric_interval_data_3447640862_2017-08-15_to_2019-08-04.csv"
gasUsageDaily = "pge_gas_interval_data_3447640427_2014-07-04_to_2019-08-04.csv"
solarProductionDaily = "Enphase_677782_system_energy_20150708_to_20190808.csv"
solarProductionHourly = "677782_system_power_20190802.csv"

pgeElectricBilling = "pge_electric_billing_data_3447640862_2014-06-27_to_2019-08-19.csv"
pgeGasBilling = "pge_gas_billing_data_3447640427_2014-06-27_to_2019-08-05.csv"
...
solarInvestment  = 6513  # $$ cost of  solar upfront investment post tax credit
...
# Figure out the  production by type (peak/offpeak/partPeak) so you can 
...
getTimeOfDayPercentages <- function() {
  ...
  setDT(solarHourlyDf)[(wday == "Saturday" | wday == "Sunday") & (hour >= 17 & hour < 20 ),
                     type := "partPeak"]  
  ...
  winterPercent = as_tibble(list(type=c("peak", "partPeak", "offPeak"),
...
addMissingTimeValues  <- function(df, dateColumnName, valueColumnName) {
  ...
  fixedDf = data.frame(as.Date(index(df2)),coredata(df2))
...
# Utility function to nest multiple attributes
...
nest_AddMissingRows_xts <- function(df_in, nest_var, t_var, value_var) {
  ...
  require(rlang)
  dfNest = df_in %>% group_by(.dots = nest_var) %>%  nest(.key = "data.tbl")
...
(ggplot(solarDf, aes(x = DATE, y = energyProducedWh)) + geom_point(color = "green") +
    ggtitle("Solar generation")) %>% ggplotly()
...
# ts leaves out a few readings because of decimal_date of the ts and maybe the leap year
# ignore for now, try not to use (1493 vs 1487)
solarTs = ts(solarDf$energyProducedWh, start=c(solarDf$year[[1]], solarDf$yDay[[1]]), frequency = 365)
comp = decompose(solarTs)
plot(comp)
...
p %>% ggplotly()
# api_create(p) - file too big for Plotly hosting
...
# Model  scenarios, two increasing electricity prices, another flat 
...
(ggplot() +
    #geom_line(data = s3, aes(x = DATE, y = monthlyUsage, colour = context)) +
    geom_line(data = s3, aes(x = DATE, y = monthlyGen)) +
    geom_line(data = solarDf, aes(x = DATE, y=TMIN)) + 
    geom_line(data = solarDf, aes(x = DATE, y=TMAX)) + 
    geom_line(data = solarDf, aes(x = DATE, y=PRCP,colour = "PRCP")) +
